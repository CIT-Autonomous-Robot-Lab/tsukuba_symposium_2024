...
計算機は、ノートPCとRaspberry Piの2台が搭載されている。
...
%技術的な課題が解決しておらず、
%ノートPCを使用することとなった。
ノートPCでは、ナビゲーションのプログラムが実行され、
Raspberry Piで車体の制御を行っている。
センサ構成としては、
各チームの機体に外界センサとして3次元LiDAR（Livox Mid-360）、
内界センサとしてIMU（アールティ社 RT-USB-9axisIMU2）を搭載した。
%%%上の記述はセンサの詳細知らないので怪しいどっちもIMU使ってたっけ%%%
